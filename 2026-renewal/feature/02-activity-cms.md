...
## 2. 해결하고자 하는 문제 (As-Is)
- **개발자 종속성:** 오타 수정이나 새로운 연도의 스터디를 추가하기 위해 매번 개발자의 PR 및 배포가 필요함.
- **제한된 확장성:** 각 활동별로 필요한 정보가 다를 수 있음에도 불구하고(예: 세미나에는 '발표자' 필드가 추가로 필요함), 기존 코드는 고정된 인터페이스(`ActivityItem`)를 사용하여 확장이 어려움.
- **로컬 스토리지 한계:** 대용량 이미지 자원을 소스 코드와 함께 관리하거나, 직접 서버로 전송하여 부하를 줌.
- **이미지 관리의 비효율성:**
